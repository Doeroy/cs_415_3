...
void* car_func(void* arg) {
    ...
    while (!shutdown_flag) {
        ...
        shared_state->car_statuses[car_id] = LOADING;
        ...
        if(is_running) {
             pthread_mutex_lock(&shared_state->lock);
             shared_state->car_statuses[car_id] = RUNNING;
             pthread_mutex_unlock(&shared_state->lock);
        } else {
             pthread_mutex_lock(&shared_state->lock);
             shared_state->car_statuses[car_id] = WAITING;
             pthread_mutex_unlock(&shared_state->lock);
        }
        ...
        if (is_running) {
            ...
            pthread_mutex_lock(&shared_state->lock);
            shared_state->car_statuses[car_id] = UNLOADING;
            pthread_mutex_unlock(&shared_state->lock);
            ...
            pthread_mutex_lock(&shared_state->lock);
            shared_state->passengers_in_car[car_id] = 0;
